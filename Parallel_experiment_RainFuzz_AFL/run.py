...
def parse_arguments():
    parser = argparse.ArgumentParser(description="RainFuzz-AFL++ script for parallel testing.")
...
if __name__=="__main__":
    ...
    print("Running RainFuzz and AFL++ for %d seconds..." % args.execution_time)
    # Starting RainFuzz
    rainfuzz_bin_path = normpath(join(rainfuzz_cpy_path, FUZZER))
    cmd_rainfuzz = [rainfuzz_bin_path, "-d", "-i", corpus_cpy_path, "-o", output_dir, "-M", "RainFuzz", "-m", "none", "--", binary_cpy_path]
    # Starting AFL++
    afl_fuzz_path = normpath(join(alfplusplus_path_cpy_path, FUZZER))
    cmd_afl = [afl_fuzz_path, "-d"]
    for dictionary in dictionaries:
        cmd_afl += ["-x", dictionary]
    cmd_afl += ["-i", corpus_cpy_path, "-o", output_dir, "-S", "AFLplusplus", "-m", "none", "--", binary_cpy_path]
    print("Command RainFuzz: %s" % " ".join(cmd_rainfuzz))
    print("Command AFL++: %s" % " ".join(cmd_afl))
    sleep(SLEEP_TIME)
    with Popen(cmd_rainfuzz) as rainfuzz_p:
        try:
            aflplusplus_p = Popen(cmd_afl, stdout=DEVNULL, stderr=DEVNULL)
            rainfuzz_p.wait(timeout=args.execution_time)
        except TimeoutExpired:
            rainfuzz_p.send_signal(SIGINT)
            aflplusplus_p.send_signal(SIGINT)
            print("RainFuzz and AFL++ killed by timeout.")
        except:  
            rainfuzz_p.kill()
            aflplusplus_p.kill()
            raise 
