...
def parse_arguments():
    parser = argparse.ArgumentParser(description="AFL++ script for testing.")
...
if __name__=="__main__":
    ...
    mkdir(output_dir)
    # Starting AFL++
    print("Running AFL++ for %d seconds..." % args.execution_time)
    afl_fuzz_path = normpath(join(alfplusplus_path_cpy_path, FUZZER))
    cmd = [afl_fuzz_path, "-d"]
    for dictionary in dictionaries:
        cmd += ["-x", dictionary]
    cmd += ["-i", corpus_cpy_path, "-o", output_dir, "-m", "none", "--", binary_cpy_path]
    print("Command: %s" % " ".join(cmd))
    sleep(SLEEP_TIME)
    with Popen(cmd) as aflplusplus_p:
        try:
            aflplusplus_p.wait(timeout=args.execution_time)
