...
for i in range(1, n_experiments + 1):
    ...
    for line in file.readlines()[1:]:
        ...
        if o >= len(old_avg_x) or old_avg_x[o] > time:
            new_avg_x.append(time)
            new_avg_y.append(((last_old_avg*(i-1))+edges_found)/i)
            if i == 2:
                print(time, ((last_old_avg*(i-1))+edges_found)/i)
        else:
            while o < len(old_avg_x) and time >= old_avg_x[o]:
